...
class TestEmployeeRelationsManager:
    ...
    def test_team_leader_john_doe(self):
        ...
        assert john_doe_leader, "John Doe is not the team leader!"
    ...
    def test_john_doe_team_memebers(self):
        ...
        assert actual_team_members == expected_team_members, "John Doe's team members are not as expected!"

    def test_tomas_andre_not_in_john_doe_team(self):
        john_doe_team_members = self.relations_manager.get_team_members(self.john_doe)
        assert 5 not in john_doe_team_members, "Tomas Andre should not be in John Doe's team!"
    ...
    def test_gretchen_walford_base_salary(self):
        ...
        assert gretchen_walford_base_salary == expected_base_salary, "Gretchen Walford's salary is not $4000!"

    def test_tomas_andre_not_team_leader(self):
        is_tomas_andre_leader = self.relations_manager.is_leader(self.tomas_andre)
        assert not is_tomas_andre_leader, "Tomas Andre should not be a teamleader!"
    ...
    def test_retrieve_tomas_andre_team_members(self):
        ...
        assert len(tomas_andre_team_members) == 0, "Tomas Andre should not have any team members"
    ...
    def test_judge_overcash_not_in_database(self):
        ...
        assert not jude_overcash_exists, "Jude Overcash should not be stored in database!"